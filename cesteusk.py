...
logger = logging.getLogger('cesteusk')

STATE_FILE = "cesteusk_state.json"

class CesteUSKScraper:
    def __init__(self):
        self.source_name = "Ceste USK"
        self.source_id = "ceste_usk_001"
        ...
        self.urls = [
            {"url": "https://cesteusk.com/", "type": "novosti"},
            {"url": "https://cesteusk.com/kategorija/konkursi/", "type": "konkursi"},
            {"url": "https://cesteusk.com/javne-nabavke/", "type": "javne_nabavke"}
        ]
        ...
        for site in self.urls:
            try:
                ...
                if not articles_elem:
                    articles_elem = soup.select('.post, .item, .vest, .konkurs, .nabavka')
    ...
    def extract_article(self, elem, content_type):
        try:
            ...
            if url and not url.startswith('http'):
                url = f"https://cesteusk.com{url}" if url.startswith('/') else f"https://cesteusk.com/{url}"
            ...
            if img_elem:
                ...
                if image_url and not image_url.startswith('http'):
                    image_url = f"https://cesteusk.com{image_url}" if image_url.startswith('/') else f"https://cesteusk.com/{image_url}"
            
            # Format based on type
            type_formats = {
                'novosti': ('', 'Novost / Obavještenje'),
                'konkursi': ('', 'Konkurs / Javni poziv'),
                'javne_nabavke': ('', 'Javna nabavka')
            }
            
            prefix, type_label = type_formats.get(content_type, ('', 'Objava'))
            
            full_content = f"{type_label} - Ceste USK\n\n{content}\n\nIzvor: JP Ceste USK d.o.o. Bihać"
