...
logger = logging.getLogger('radiovkladusa')

STATE_FILE = "radiovkladusa_state.json"

class RadioVKladusaScraper:
    def __init__(self):
        self.source_name = "Radio V Kladuša"
        self.source_id = "radio_v_kladusa_001"
        ...
        self.urls = [
            {"url": "https://www.radiovkladusa.ba/", "type": "naslovna"},
            {"url": "https://www.radiovkladusa.ba/kategorija/servisne-informacije/", "type": "servisne"}
        ]
    ...
    def extract_article(self, elem, content_type):
        try:
            ...
            if url:
                if url.startswith('/'):
                    url = f"https://www.radiovkladusa.ba{url}"
                elif not url.startswith('http'):
                    url = f"https://www.radiovkladusa.ba/{url.lstrip('/')}"
            ...
            if img_elem:
                ...
                if image_url and image_url.startswith('/'):
                    image_url = f"https://www.radiovkladusa.ba{image_url}"
            
            # Format based on type
            prefix = ""
            type_label = "V. Kladuša" if content_type == 'naslovna' else "Servisna informacija"
            
            full_content = f"{prefix} Radio V Kladuša - {type_label}\n\n{content}\n\nIzvor: Radio Velika Kladuša"
